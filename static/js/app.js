/**
 * ESIMPEG - Reusable JavaScript Functions Library
...
try {
    onReady(function(){
        try {
            if (document.getElementById('esimpeg-modal-open-hide-dt-style')) return;
            const st = document.createElement('style');
            st.id = 'esimpeg-modal-open-hide-dt-style';
            st.type = 'text/css';
            st.textContent = `
              body.esimpeg-modal-open .dataTables_processing { display: none !important; }
              body.esimpeg-modal-open #table-loading-overlay { display: none !important; }
            `;
...
(function initGlobalLoadingOverlay(){
    try {
        if (window.__ESIMPEG_GLOBAL_LOADING_INIT__) return;
        window.__ESIMPEG_GLOBAL_LOADING_INIT__ = true;
        ...
        document.body.addEventListener('htmx:beforeRequest', function(evt){
            try {
                if (window.__ESIMPEG_NO_LOADER_NEXT) {
                    window.__ESIMPEG_NO_LOADER_NEXT = false;
                    return;
                }
                ...
                const noLoaderByHeader = hdrs && (hdrs['X-ESIMPEG-No-Loader'] === '1' || hdrs['x-esimpeg-no-loader'] === '1');
...
function ajaxFormSubmitWithSwal(formId, options = {}) {
    ...
    const config = {
        ...
        showUsername: options.showUsername !== undefined ? options.showUsername : null,
...
// ==========================================
// Reusable binder: SweetAlert for HTMX form events
// ==========================================
function bindFormSwal(container, options = {}) {
    ...
    function onSuccess(e) {
        try {
            ...
            if (url) {
                try {
                    sessionStorage.setItem('__esimpeg_toast_next__', JSON.stringify({ message: msg, type: 'success' }));
                } catch (_) {}
                if (window.Swal) {
                    Swal.fire({
                        icon: 'success',
                        ...
                        try { window.location = url; } catch (_) { window.location.href = url; }
                    });
                } else {
                    try { window.location = url; } catch (_) { window.location.href = url; }
                }
            } else if (typeof window.showToast === 'function') {
                try { window.showToast(msg, 'success'); } catch(_) {}
            } else if (window.Swal) {
                Swal.fire({
                    icon: 'success',
                    title: 'Berhasil',
    ...
    selects.forEach(function(sel){
        ...
        function onViewportChange(e){
            try {
                ...
                if (e && e.type === 'scroll') {
                    const timeSinceOpen = Date.now() - openTimestamp;
                    if (timeSinceOpen < minOpenDuration) {
                        return;
                    }
                    positionDropdown();
                    try {
                        ...
                        const topbarH = 60;
                        if (r.top < topbarH || r.bottom < 0 || r.top > window.innerHeight) {
                            close();
                            return;
                        }
                    } catch(_) {}
